...
function isInsideCircle(circle, point) {
  return ((circle.center.y - point.y) ** 2) + (circle.center.x - point.x) ** 2
   < (circle.radius) ** 2;
}
...
function findFirstSingleChar(str) {
  for (let j = 0; j < Math.round(str.length / 2); j += 1) {
    if (str.slice(j + 1).indexOf(str[j]) === -1) {
      return str[j];
    }
  }
  return null;
...
function isCreditCardNumber(ccn) {
  let res = 0;
  for (let i = 1; i <= ccn.toString().length; i += 1) {
    let a = Number(ccn.toString()[ccn.toString().length - i]);
    if (i % 2 === 0) {
      a *= 2;
    }
    if (a > 9) {
  ...
  for (let j = 0; j < pathes[0].length; j += 1) {
    for (let i = 1; i < pathes.length; i += 1) {
      if (pathes[i][j] !== pathes[0][j]) {
        const sI = res.lastIndexOf('/');
        return res.slice(0, sI + 1);
      }
    }
    res += pathes[0][j];
  }
...
function getMatrixProduct(m1, m2) {
  ...
  const m = new Array(m1R);
  for (let i = 0; i < m1R; i += 1) {
    m[i] = new Array(m2C);
    for (let j = 0; j < m2C; j += 1) {
      m[i][j] = 0;
      for (let k = 0; k < m1C; k += 1) {
        m[i][j] += m1[i][k] * m2[k][j];
      }
    }
  }
